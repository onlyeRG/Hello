...
async def add_pubchannel(client, message):
    try:
        ...
        try:
            ...
            await logger.log_error(client, "ChannelAccessError", user_id, str(e))
            return
        ...
        if success:
            ...
            await logger.log_channel_added(client, user_id, channel_id, base_username, interval)
        else:
            await message.reply(f"<b>❌ Error starting rotation:</b> {result}")
            await logger.log_error(client, "RotationStartError", user_id, result)
    except ValueError:
        ...
        await logger.log_error(client, "AddChannelError", message.from_user.id, str(e))
...
async def stop_channel(client, message):
    try:
        ...
        if success:
            ...
            await logger.log_channel_stopped(client, user_id, channel_id)
        else:
            await message.reply(f"<b>❌ Error:</b> {result}")
            await logger.log_error(client, "StopChannelError", user_id, result)
    except ValueError:
        await message.reply("<b>Invalid channel ID.</b>")
    except Exception as e:
        await message.reply(f"<b>Error:</b> {str(e)}")
        await logger.log_error(client, "StopError", message.from_user.id, str(e))
...
async def resume_channel(client, message):
    try:
        ...
        if success:
            ...
            await logger.log_channel_resumed(client, user_id, channel_id)
        else:
            await message.reply(f"<b>❌ Error:</b> {result}")
            await logger.log_error(client, "ResumeChannelError", user_id, result)
    except ValueError:
        await message.reply("<b>Invalid channel ID.</b>")
    except Exception as e:
        await message.reply(f"<b>Error:</b> {str(e)}")
        await logger.log_error(client, "ResumeError", message.from_user.id, str(e))
...
async def logout_all(client, message):
    ...
    async for user in users:
        if user.get('session'):
            ...
            await logger.log_user_logout(client, user['id'])
            count += 1
